...
class BtcWallet {
   ...
   * @param  {Boolean} params.sendMax                (optional)  Send all the money that you have in your address, if true then ignore amount
  ...
  async createRawTx(params = {}) {
    ...
    const { inputs, outputs, fee } = params.sendMax ? this._coinSelectSendMax(inputTxs, targets, feePerB) : coinSelect(inputTxs, targets, feePerB);
    if (!inputs || !outputs) {
      return {
        raw: '',
        fee,
