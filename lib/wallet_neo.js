...
class NeoWallet extends Wallet {
  ...
  async getBalance(assetId) {
    let result = await this.infinitApi.getBalance(this.Account.address, assetId);
 ...
 */
  async transfer(contractAddress, to, amount, decimal = Constant.NEO.GASDecimal) {
    let rawTx = await this.createTransferTx(contractAddress, to, amount, decimal);
  ...
  async createRawTx(params) {
    ...
    let outputs = [];
    const inputTxs = await this._getUnspendList(params.assetId);
 ...
 * @returns {String}  
 */
  async  createTransferTx(contractAddress, to, amount, decimal = Constant.NEO.GASDecimal) {
    ...
    const preTransferAmount = parseFloat(new BigNumber(amount).times(decimal));
    ...
    const fromAddrScriptHash = Neon.wallet.getScriptHashFromAddress(this.Account.address);
    const toAddrScriptHash = Neon.u.reverseHex(Neon.wallet.getScriptHashFromAddress(to))
    ...
    const intents = [
      {
        assetId: Constant.NEO.GASAssetId, // GAS AssetId
        value: 0.00000001,
  ...
  async _getUnspendList(assetId) {
    ...
    let result = await this.infinitApi.getUtxo(this.Account.address, assetId);
    if (result.cd != 0) {
      return inputTxs;
    }
