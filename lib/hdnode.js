...
class HDNode {
  ...
  generateKeyPair(coinType, index, isTestNet) {
    ...
    if (!this.hdPath) {
      ...
      let parts = this.hdPath.split('/');
      if (parts.length != 6) {
        throw new AppError(
          Util.format(Messages.invalid_hdpath.message, this.hdPath),
          Messages.invalid_hdpath.code
        );
      }
      parts[5] = index;
      this.hdPath = parts.join('/');
    }
    if (!this.mnemonic) this.mnemonic = HDNode.generateMnemonic();
