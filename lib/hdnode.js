...
class HDNode {
  ...
  generateKeyPair(coinType, index, isTestNet) {
    ...
    if (!this.hdPath) {
      let coinIndex = isTestNet
        ? this.getCoinIndexBip44('TESTNET')
        : this.getCoinIndexBip44(coinType.toUpperCase());
      if (!coinIndex) {
        throw new AppError(
    ...
    if (!this.mnemonic) this.mnemonic = this.generateMnomenic();
  ...
  static getNetwork(coinType, isTestnet = false) {
    ...
    switch (coinType) {
      case CoinType.BTC.symbol: {
        network = isTestnet
          ? CoinType.BTC.network.testnet
          : CoinType.BTC.network.mainnet;
        break;
      }
      case CoinType.BCH.symbol: {
        network = isTestnet
          ? CoinType.BCH.network.testnet
          : CoinType.BCH.network.mainnet;
        break;
      }
      ...
      case CoinType.LTC.symbol: {
        network = isTestnet
          ? CoinType.LTC.network.testnet
          : CoinType.LTC.network.mainnet;
        break;
      }
      case CoinType.DASH.symbol: {
        network = isTestnet
          ? CoinType.DASH.network.testnet
          : CoinType.DASH.network.mainnet;
        break;
      }
      case CoinType.DOGE.symbol: {
        network = isTestnet
          ? CoinType.DOGE.network.testnet
          : CoinType.DOGE.network.mainnet;
        break;
      }
    }
