...
const _getDataSmartContract = Symbol('getDataSmartContract');
 ...
 */
  ...
  async getTxAddress(offset, limit) {
    let result = await this.CoinApi.getTxAddress(this.Account.address, offset, limit);
  ...
  async getInternalTxAddress(offset, limit) {
    let result = await this.CoinApi.getInternalTxAddress(this.Account.address, offset, limit);
    return this._getReponse(result);
  }


  /**
   * @returns {Object} { total: 0, from: 0, to: 0, contracts: [{ address: '',name: '',symbol: '', decimals: 18,total_supply: '',offical_site: '',is_verified: true, is_popular: true }] }
   */
  async getContract() {
    let result = await this.CoinApi.getContract();
    return this._getReponse(result);
  }
  ...
  async getContractHistory(scAddress, offset, limit) {
    let result = await this.CoinApi.getContractHistory(scAddress, offset, limit, this.Account.address);
    return this._getReponse(result);
  }
  ...
  async getContractBalance(scAddress) {
    let result = await this.CoinApi.getContractBalance(scAddress, this.Account.address);
    return this._getReponse(result);
  }
  ...
  async createRawTx(txParams) {
    ...
    if (txParams.data) {
      ...
      let _data = this[_getDataSmartContract](txParams.sc.nameFunc, txParams.sc.typeParams, txParams.sc.paramsFuncs);
  ...
  [_getDataSmartContract](nameFunction, typeParams, Params) {
