...
function generateAccount(privateKey, isTestNet = false) {
  let network = isTestNet
    ? BitcoreLibCash.Networks.testnet
    : BitcoreLibCash.Networks.mainnet;
  let keyPair = new BitcoreLibCash.PrivateKey(privateKey, network);

  let address = keyPair.toAddress();
  let legacyAddress = BchAddrJs.toLegacyAddress(address.toLegacyAddress());
  let bitpayAddress = BchAddrJs.toBitpayAddress(legacyAddress);

  return {
    address: legacyAddress,
    legacyAddress: legacyAddress,
    cashAddress: address.toCashAddress(),
