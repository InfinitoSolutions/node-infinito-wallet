...
/**
 * @param  {String}  wifkey              (required) coin's private key
 * @param  {Object}  network                 (required) coin's network
 * @param  {Boolean} isTestNet=false         (optional) Environment type (default = false))
 */
function generateAccount(wifkey, network, isTestNet = false) {
  let keyPair = Bitcoinjs.ECPair.fromWIF(wifkey, network);
