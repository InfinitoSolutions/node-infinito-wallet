...
class Wallet {
  ...
  constructor(options) {
    ...
    this.Account = new BitcoinAccount({
  ...
  _check(options) {
    ...
    if (!options || !options.mnemonic || !options.privateKey || !options.coinType) {
      throw new AppError(Util.format(Messages.missing_parameter.message), Messages.missing_parameter.code);
    }
  }
}
