...
describe('hdnode', async() => {
  ...
  describe('#generateKeyPair()', async() => {
    it('Generated passphase', async() => {
      hdNode = new HDNode();
      hdNode.generateKeyPair("BTC", 0, true);
  ...
  describe('#getHDPath()', async() => {
    it('Default hdpath', async() => {
      hdNode = new HDNode();
      let hdpath = hdNode.getHDPath();
      Assert.ok(hdpath === undefined, "Must be undefined");
    });
  ...
  describe('#getWif() getPublicKey() getPrivateKey()', async() => {
    it('BTC', async() => {
      ...
      hdNode = new HDNode({ mnemonic: "toss true onion like penalty spot cloth need disease start coyote suggest" });

      let index = 0;
      keymaps.forEach((keymap) => {
        // console.log('Index: ', index);
        // console.log('keymap :', keymap);
        // console.log('getWif :', hdNode.getWif());
        // console.log('getPublicKey :', hdNode.getPublicKey().toString("hex"));
        // console.log('getPrivateKey :', hdNode.getPrivateKey().toString("hex"));
        hdNode.generateKeyPair("BTC", index, false);
    ...
    it('Testnet', async() => {
      ...
      let index = 0;
      keymaps.forEach((keymap) => {
        hdNode.generateKeyPair("BTC", index, true);
        Assert.equal(hdNode.getHDPath(), keymap[0], `HdPath must be equal. ${keymap[0]} - Index: ${index}`);
        Assert.equal(hdNode.getWif(), keymap[3], `Wif must be equal. ${keymap[0]} - Index: ${index}`);
        Assert.equal(hdNode.getPublicKey().toString("hex"), keymap[2], `Public key must be equal. ${keymap[0]} - Index: ${index}`);
        index++;
  ...
  describe('#getMnemonic()', async() => {
    it('Default hdpath', async() => {
      ...
      Assert.ok(mnemonic === undefined, "Must be undefined");
    });
    it('Pass parameter', async() => {
      hdNode = new HDNode({ mnemonic: "toss true onion like penalty spot cloth need disease start coyote suggest" });
      let mnemonic = hdNode.getMnemonic();
